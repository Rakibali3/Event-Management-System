...
app.get('/home', connectEnsureLogin.ensureLoggedIn(), (req, res) => {
  ...
  client.query('select * from  public."Addevents"', (err, result) => {
    if (!err) {
      ...
      res.render(__dirname + '/views/card.ejs', { csrfToken: req.csrfToken(), Data: data, message: req.flash('success'), message: req.flash('delete'), message: req.flash('cancel'), count: count, userid: req.user.id });
    } else {
      console.log(err.message);
    }
    client.end;
...
app.post('/cancel_:id', async (req, res) => {
  ...
  try {
    ...
    //deleting registeration of user
    ...
    req.flash('cancel', 'u have successfully canceled ur registration');
    res.redirect("/home");
  } catch (error) {
...
app.post("/contacts",async (req,res)=>{
  ...
  console.log(`created task with Id: ${newContact.id}`);
  res.redirect('/home');
